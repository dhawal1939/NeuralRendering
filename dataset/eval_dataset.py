...
#             return uv_map.type(torch.float), torch.from_numpy(extrinsics).type(torch.float), mask.type(torch.float),\
#                     torch.from_numpy(sh).type(torch.float), self.idx_list[idx]
#         else:
#             return uv_map, mask, sh, self.idx_list[idx]

    @staticmethod
    def _collect_fn(data, view_direction=False):
        if view_direction:
            uv_maps, extrinsics, masks, sh, idxs = zip(*data)
            ...
            uv_maps, masks, sh, idxs = zip(*data)
            uv_maps = torch.stack(tuple(uv_maps), dim=0)
            masks = torch.stack(tuple(masks), dim=0)
    ...
    def get_collect_fn(view_direction=False):
        collect_fn = lambda x: EvalDatasetSH._collect_fn(x, view_direction)
        return collect_fn
