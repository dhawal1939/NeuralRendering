...
from util import augment, augment_eval,augment_og, augment_center_crop
...
class UVDatasetSHLow(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = ImageOps.grayscale(mask)
        forward = Image.open(os.path.join(self.dir, 'forward/'+self.idx_list[idx]+'.png'), 'r')
        sh = np.transpose( np.load(os.path.join(self.dir, 'sh/'+self.idx_list[idx]+'.npy')), (2, 0, 1) )[:27]
        # print(sh.shape)
        uv_map = np.load(os.path.join(self.dir, 'uv/'+self.idx_list[idx]+'.npy'))
        nan_pos = np.isnan(uv_map)
        uv_map[nan_pos] = 0
