...
import config
from dataset.uv_dataset import UVDataset, UVDatasetSH, UVDatasetMask
from model.pipeline import PipeLine, PipeLineSH,PipeLineMask
...
def main():
   ...
   # print(model_lif)
    ...
    for i in range(1, 1+args.mask_epoch):
        ...
        # adjust_learning_rate(optimizer_mask, i, args.lr)
    ...
    for i in range(1, 1+args.epoch):
        print('Epoch {}'.format(i))
        # adjust_learning_rate(optimizer_lif, i, args.lr)
        model_lif.train()
        torch.set_grad_enabled(True)
