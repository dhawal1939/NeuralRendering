...
def main():
    ...
    dataset = UVDataset(args.data+'/train/', args.train, args.croph, args.cropw, args.view_direction)
    dataloader = DataLoader(dataset, batch_size=args.batch, shuffle=True, num_workers=4)

    testdataset = UVDataset(args.data+'/test/',args.train, args.croph, args.cropw, args.view_direction)
    test_dataloader = DataLoader(testdataset,batch_size=1,shuffle=True,num_workers=4)
    test_step=0
    ...
    for i in range(1, 1+args.epoch):
        ...
        print('loss at epoch {}: {}'.format(step, loss.item()/len(dataset)))
        ...
        for samples in tqdm(test_dataloader):
            images, uv_maps, extrinsics, masks = samples
            test_step += images.shape[0]
            RGB_texture, preds_final = model(uv_maps.cuda(), extrinsics.cuda())

            tloss1 = criterion(RGB_texture.cpu(), images)
            tloss2 = criterion(preds_final.cpu(), images)
            t1_loss = tloss1 + tloss2
            # t1_loss = criterion(preds_final, images.cuda())
            test_loss += t1_loss.item()

            output = np.clip(preds_final[0, :, :, :].detach().cpu().numpy() ** (1.0/2.2), 0, 1)
            output = output * 255.0
            output = output.astype(np.uint8)

            gt = np.clip(images[0, :, :, :].numpy() ** (1.0/2.2), 0, 1)
            gt = gt * 255.0
        ...
        # ridx = random.randint(0, len(testdataset)-1)
        ridx = i%len(testdataset)
        writer.add_scalar('test/loss', test_loss/len(testdataset), test_step)
